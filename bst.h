...
protected:
    ...
    void delete_tree(Node<Key, Value>* tree);
    std::pair<int, bool> balanced_helper(Node<Key, Value>* current) const;
...
typename BinarySearchTree<Key, Value>::iterator& BinarySearchTree<Key, Value>::iterator::operator++() {
    ...
    /*if has right child, find the node with smallest key in the right subtree
     else go up along the ancestor tree and find the first node whose left child is in the line
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value>& keyValuePair) {
    ...
    // if tree is empty, creat a root
    if (empty()) {
        Node<Key, Value>* newItem = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, NULL);
        root_ = newItem;
        return;
    }
    ...
    // creat new node and adjust pointers
    Node<Key, Value>* newItem = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, temp);

    if (keyValuePair.first > temp->getKey()) {
        temp->setRight(newItem);
        newItem->setParent(temp);
    } else {
...
void BinarySearchTree<Key, Value>::remove(const Key& key) {
    ...
    // find the node, if cannot find, do nothing
...
Node<Key, Value>* BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current) {
    ...
    /*if has left child, find the node with biggest key in the right subtree
     else go up along the ancestor tree and find the first node whose right child is in the line
     return NULL if no oredecessor found*/
...
/**
 * A helper function to delete all nodes in the tree recursively
 */
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::delete_tree(Node<Key, Value>* tree) {

    if (!tree)
        return;

    // delete left subtree
    delete_tree(tree->getLeft());
    // delete right subtree
    delete_tree(tree->getRight());
...
bool BinarySearchTree<Key, Value>::isBalanced() const {
    ...
    // call helper function
    return balanced_helper(root_).second;
}

/**
 * A helper function for isBalanced function
 */
template<typename Key, typename Value>
std::pair<int, bool> BinarySearchTree<Key, Value>::balanced_helper(Node<Key, Value>* current) const {
    ...
    std::pair<int, bool> L_tree(0, true);
    std::pair<int, bool> R_tree(0, true);

    if (current->getLeft() != NULL) {
        L_tree = balanced_helper(current->getLeft());
    }

    if (current->getRight() != NULL) {
        R_tree = balanced_helper(current->getRight());
    }

    pair.first = std::max(L_tree.first, R_tree.first) + 1;

    if (std::abs(L_tree.first - R_tree.first) <= 1 && L_tree.second && R_tree.second) {
        pair.second = true;
    }
